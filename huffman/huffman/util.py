...
def decompress(compressed, uncompressed):
    ...
    while True:
        ...
        '''in order to exclude the EOF from being
...
def compress(tree, uncompressed, compressed):
    ...
    nonebytes = encoding_table[None]
    try:
        while True:
            bit = bitreader.readbits(8)
            if bit is None:
                raise EOFError
            bytes = encoding_table[bit]
            for single in bytes:
                bitwriter.writebit(single)
            
    except EOFError:
        '''adding the EOF bits to the compressed stream
           when end of stream/fileis encountered'''
        for single in nonebytes:
            bitwriter.writebit(single)
    bitwriter.flush()
